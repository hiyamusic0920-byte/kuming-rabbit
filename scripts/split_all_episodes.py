...
EPISODE_CROPS: dict[int, list[tuple[int, int, int, int]]] = {
    ...
    3: [
        (5, 52, 461, 432), (471, 52, 943, 432), (953, 52, 1524, 439),
        (5, 441, 411, 925), (421, 441, 1063, 925), (1071, 441, 1524, 925),
    ],
    4: [
...
# Episode 10 spans two source images with different dimensions and coordinates.
EPISODE_10_PAGES: list[
    tuple[str, list[tuple[int, int, int, int]]]
] = [
    ("full.jpg", EPISODE_CROPS[10]),
    (
        "full-2.jpg",
        [
            (8, 47, 771, 527), (779, 47, 1540, 527),
            (8, 535, 771, 1009), (779, 535, 1540, 1009),
        ],
    ),
]
...
# Episode 15 is already split across four standalone source pages.
EPISODE_15_PAGES: list[str] = [
    "full.jpg",
    "full-2.jpg",
    "full-3.jpg",
    "full-4.jpg",
...
def split_episode_10() -> list[Path]:
    episode = 10
    episode_dir = ASSETS_DIR / "ep10"
    panels: list[Image.Image] = []
    output_paths: list[Path] = []

    for source_name, crop_boxes in EPISODE_10_PAGES:
        source_path = episode_dir / source_name
        if not source_path.is_file():
            raise FileNotFoundError(f"Source image not found: {source_path}")
        with Image.open(source_path) as source:
            for crop_box in crop_boxes:
                ...
                if not (
                    ...
                    raise ValueError(f"ep10 invalid crop in {source_name}: {crop_box}")
                panel_number = len(panels) + 1
                panel = source.crop(crop_box)
                output_path = episode_dir / f"{panel_number:02d}.png"
                panel.save(output_path, "PNG")
    ...
    create_contact_sheet(panels, EPISODE_ROWS[episode]).save(sheet_path, "PNG")
    output_paths.append(sheet_path)
    print(f"ep10: {len(panels)} panels")
    return output_paths
...
def split_episode_15() -> list[Path]:
    ...
    for panel_number, source_name in enumerate(EPISODE_15_PAGES, start=1):
        source_path = episode_dir / source_name
        if not source_path.is_file():
            raise FileNotFoundError(f"Source image not found: {source_path}")
        with Image.open(source_path) as source:
    ...
    create_contact_sheet(panels, EPISODE_ROWS[episode]).save(sheet_path, "PNG")
    output_paths.append(sheet_path)
    print(f"ep15: {len(panels)} panels")
    return output_paths
